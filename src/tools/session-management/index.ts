...
export function registerSessionTools(): Tool[] {
  return [
    ...
    },
    {
      ...
      description: 'Delete all chat messages from a session',
      inputSchema: {
        type: 'object',
        properties: {
          sessionId: {
    ...
    },
    {
      ...
      description: 'Delete all Q&A data from a session',
      inputSchema: {
        type: 'object',
        properties: {
          sessionId: {
    ...
    },
    {
      ...
      description: 'Delete all summaries from a session',
      inputSchema: {
        type: 'object',
        properties: {
          sessionId: {
    ...
    },
    {
      ...
      description: 'Delete all polls from a session',
      inputSchema: {
        type: 'object',
        properties: {
          sessionId: {
...
export async function executeSessionTool(
  ...
  switch (toolName) {
    ...
    case 'delete-session-chats': {
      logger.info('Deleting session chats', { sessionId: args.sessionId });
      
      try {
        await apiClient.deleteSessionData(args.sessionId, 'chat');
        
        return {
          content: [{
            type: 'text',
            text: `Successfully deleted all chat messages for session ${args.sessionId}`,
          }],
        };
      } catch (error) {
    ...
    case 'delete-session-qa': {
      logger.info('Deleting session Q&A', { sessionId: args.sessionId });
      
      try {
        await apiClient.deleteSessionData(args.sessionId, 'questions');
        
        return {
          content: [{
            type: 'text',
            text: `Successfully deleted all Q&A data for session ${args.sessionId}`,
          }],
        };
      } catch (error) {
    ...
    case 'delete-session-summaries': {
      logger.info('Deleting session summaries', { sessionId: args.sessionId });
      
      try {
        await apiClient.deleteSessionData(args.sessionId, 'summaries');
        
        return {
          content: [{
            type: 'text',
            text: `Successfully deleted all summaries for session ${args.sessionId}`,
          }],
        };
      } catch (error) {
    ...
    case 'delete-session-polls': {
      logger.info('Deleting session polls', { sessionId: args.sessionId });
      
      try {
        await apiClient.deleteSessionData(args.sessionId, 'polls');
        
        return {
          content: [{
            type: 'text',
            text: `Successfully deleted all polls for session ${args.sessionId}`,
          }],
        };
      } catch (error) {
