 ...
 * @returns {void}
 */
export function setupRecordingResources(
  server: McpServer,
  apiUrl: string,
): void {
      ...
      try {
        ...
        if (!apiKey) {
          throw new AuthenticationError(
            "API key is required to access recordings",
          );
        }
        ...
        logger.error("Error in recordings resource", {
  ...
  server.resource(
    "recording",
    new ResourceTemplate("digitalsamba://recordings/{recordingId}", {
      list: undefined,
    }),
    async (uri, params, request) => {
      try {
        ...
        if (!apiKey) {
          throw new AuthenticationError(
            "API key is required to access recording details",
          );
        }
        ...
        const recordingIdStr = Array.isArray(recordingId)
          ? recordingId[0]
          : recordingId;
        ...
        logger.error("Error in recording resource", {
          error: error instanceof Error ? error.message : String(error),
        });
        throw error;
      }
  ...
  server.resource(
    "room-recordings",
    new ResourceTemplate("digitalsamba://rooms/{roomId}/recordings", {
      list: undefined,
    }),
    async (uri, params, request) => {
      try {
        ...
        if (!apiKey) {
          throw new AuthenticationError(
            "API key is required to access room recordings",
          );
        }
        ...
        logger.error("Error in room-recordings resource", {
          error: error instanceof Error ? error.message : String(error),
        });
        throw error;
      }
  ...
  server.resource(
    "archived-recordings",
    new ResourceTemplate("digitalsamba://recordings/archived", {
      list: undefined,
    }),
    async (uri, _params, request) => {
      try {
        ...
        if (!apiKey) {
          throw new AuthenticationError(
            "API key is required to access archived recordings",
          );
        }
        ...
        logger.error("Error in archived-recordings resource", {
          error: error instanceof Error ? error.message : String(error),
        });
        throw error;
      }
