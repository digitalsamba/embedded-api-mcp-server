...
describe('MemoryCache', () => {
  ...
  describe('set and get', () => {
    ...
    it('should store values in namespaces', () => {
      ...
      const result = cache.get(key, namespace);
    ...
    it('should not retrieve values from different namespaces', () => {
      ...
      cache.set(key, value1, 'namespace1');
      cache.set(key, value2, 'namespace2');
      
      expect(cache.get(key, 'namespace1')).toEqual(value1);
      expect(cache.get(key, 'namespace2')).toEqual(value2);
    ...
    it('should respect custom TTL for specific items', () => {
      ...
      cache.set(key2, value, undefined, customTtl);
  ...
  describe('invalidate', () => {
    ...
    it('should invalidate keys in a specific namespace', () => {
      ...
      cache.set(key, value, namespace1);
      cache.set(key, value, namespace2);
      
      // Both namespaced keys should be available
      expect(cache.get(key, namespace1)).toEqual(value);
      expect(cache.get(key, namespace2)).toEqual(value);
      ...
      expect(cache.get(key, namespace1)).toBeUndefined();
      expect(cache.get(key, namespace2)).toEqual(value);
    });
    ...
    it('should invalidate all keys with invalidateAll', () => {
      ...
      expect(cache.get(key1)).toEqual(value);
      expect(cache.get(key2)).toEqual(value);
      expect(cache.get(key1, namespace)).toEqual(value);
      ...
      expect(cache.get(key1)).toBeUndefined();
      expect(cache.get(key2)).toBeUndefined();
      expect(cache.get(key1, namespace)).toBeUndefined();
    });
  });
  ...
  describe('size limiting', () => {
    it('should respect maxItems and remove oldest items first', () => {
      ...
      for (let i = 0; i < maxItems; i++) {
        ...
        for (let j = 0; j <= i; j++) {
          expect(cache.get(`key${j}`)).toEqual({ data: `value${j}` });
        }
      }
      
      // Add one more item (exceeding maxItems)
      cache.set('key-extra', { data: 'extra' });
      ...
      expect(cache.get('key-extra')).toEqual({ data: 'extra' });
    });
    
    it('should track size per namespace', () => {
      const maxItems = 2;
      const cache = new MemoryCache({ maxItems });
      const namespace1 = 'namespace1';
      const namespace2 = 'namespace2';
      
      // Add maxItems to namespace1
      cache.set('key1', { data: 'value1' }, namespace1);
      cache.set('key2', { data: 'value2' }, namespace1);
      
      // Add maxItems to namespace2
      cache.set('key1', { data: 'value1' }, namespace2);
      cache.set('key2', { data: 'value2' }, namespace2);
      
      // All items should be available
      expect(cache.get('key1', namespace1)).toEqual({ data: 'value1' });
      expect(cache.get('key2', namespace1)).toEqual({ data: 'value2' });
      expect(cache.get('key1', namespace2)).toEqual({ data: 'value1' });
      expect(cache.get('key2', namespace2)).toEqual({ data: 'value2' });
      
      // Add one more item to namespace1
      cache.set('key3', { data: 'value3' }, namespace1);
      
      // The oldest item in namespace1 should be removed
      expect(cache.get('key1', namespace1)).toBeUndefined();
      
      // Other items should still be available
      expect(cache.get('key2', namespace1)).toEqual({ data: 'value2' });
      expect(cache.get('key3', namespace1)).toEqual({ data: 'value3' });
      expect(cache.get('key1', namespace2)).toEqual({ data: 'value1' });
      expect(cache.get('key2', namespace2)).toEqual({ data: 'value2' });
    });
  });
  ...
  describe('ETag support', () => {
    it('should generate and store ETags when useEtag is enabled', () => {
      ...
      cache.set(key, value);
      const result = cache.getWithEtag(key);
    ...
    it('should generate the same ETag for the same value', () => {
      ...
      cache.set(key, value);
      const result1 = cache.getWithEtag(key);
      
      // Set the same value again
      cache.set(key, { ...value });
      const result2 = cache.getWithEtag(key);
  ...
  describe('hasKey', () => {
    ...
    it('should return false for non-existent keys', () => {
      ...
      expect(cache.hasKey(key)).toBe(false);
    });
