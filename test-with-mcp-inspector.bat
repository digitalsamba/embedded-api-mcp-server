...
echo Starting MCP Server on port 3555...
start /B cmd /c node dist/src/index.js --log-level="debug" --port=3555 > mcp-inspector-test.log 2>&1

echo.
echo MCP Server started on port 3555
echo.
echo To test with MCP Inspector:
echo 1. Install MCP Inspector: npm install -g @modelcontextprotocol/inspector
echo 2. Run: mcp-inspector --url http://localhost:3555/mcp --header "Authorization: Bearer YOUR_API_KEY"
echo.
